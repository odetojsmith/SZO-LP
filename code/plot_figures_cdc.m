...
% data from code
load('SZO_LP/rec_cost_LP.mat');
load('SZO_LP/rec_max_con_LP.mat');
load('SZO_LP/rec_timing_LP.mat');
load('LB_SGD/rec_cost_LB.mat');
load('LB_SGD/rec_max_con_LB.mat');
load('LB_SGD/rec_timing_LB.mat');
load('SZO_QQ/rec_cost_QQ.mat')
load('SZO_QQ/rec_max_con_QQ.mat');
load('SZO_QQ/rec_timing_QQ.mat')
load('ES/rec_cost_ES.mat');
load('ES/rec_max_con_ES.mat');
load('ES/rec_timing_ES.mat');
...
figure(2)
plot(rec_cost_LP_long,'Color',clm2(1,:),'LineWidth',6); hold on
plot(rec_cost_LB_long,'Color',clm2(2,:),'LineWidth',6); hold on
plot(rec_cost_QQ_long,'Color',clm2(3,:),'LineWidth',6); hold on
plot(rec_cost_ES,'Color',clm2(4,:),'LineWidth',6); hold on
plot(ones(size(rec_cost_LB)).*800.14,'--','Color','k','LineWidth',3);
set(gca,'YTick', 810:20:870)
xlim([0,1500]);
ylim([800,881]);
xlabel('iteration','Interpreter','latex');
ylabel('generation cost','Interpreter','latex');
legend("SZO-LP","LB-SGD","SZO-QQ","Extremum-Seeking","Model-based",'Interpreter','latex','FontSize',26,'location','southeast');

set(gca,'Fontname','latex','linewidth',1.5,'FontSize',35,'TickLabelInterpreter','latex');
set(gca,'LooseInset',get(gca,'TightInset'))
set(gca,'XTick', [0 400 900 1400]);
